...
const NoteIdentification = () => {
  ...
  const winGame = () => {
    ...
    if (authTokens) {
      fetch(
        `${process.env.REACT_APP_BACK_URL}scores/${gameId}/${JSON.parse(
  ...
  return (
    <section className="note-identification">
      ...
      {!isStarted && (
        <>
          <p>
            Lors de ce jeu, tu devras identifier 20 notes, qui apparaîtrons
            chacune leur tour sur une partition.
            <br />
            Le plus rapidement possible, tu devras appuyer sur le bouton
            contenant le nom de la note.
            <br />
            <b>La clef de Sol ainsi que la clef de Fa sont présentes</b>.<br />
            Plus tu iras vite et moins tu te trompera, plus tu auras un meilleur
            score.
          </p>
      ...
      {isStarted && currentNote !== 20 && !isFinished && (
        <>
          ...
          <div className="answer-buttons">
            <button onClick={() => sendAnswer("do")} className="do">
              Do
            </button>
            <button onClick={() => sendAnswer("do#")} className="do">
              Do#
            </button>
            <button onClick={() => sendAnswer("re")} className="re">
              Ré
            </button>
            <button onClick={() => sendAnswer("re#")} className="re">
              Ré#
            </button>
            <button onClick={() => sendAnswer("mi")} className="mi">
              Mi
            </button>
            <button onClick={() => sendAnswer("fa")} className="fa">
              Fa
            </button>
            <button onClick={() => sendAnswer("fa#")} className="fa">
              Fa#
            </button>
            <button onClick={() => sendAnswer("sol")} className="sol">
              Sol
            </button>
            <button onClick={() => sendAnswer("sol#")} className="sol">
              Sol#
            </button>
            <button onClick={() => sendAnswer("la")} className="la">
              La
            </button>
            <button onClick={() => sendAnswer("la#")} className="la">
              La#
            </button>
            <button onClick={() => sendAnswer("si")} className="si">
              Si
            </button>
      ...
      {isStarted && isFinished && (
        <>
          ...
          <p>
            Bravo à toi !{" "}
            {!authTokens && (
              <>Inscris-toi pour pouvoir enregistrer tes scores</>
            )}
          </p>
        </>
      )}
